...
```
   ...
   │   Controls/    ControlBase → typed [Parameter] props + pass-through Attributes    │
...
three solution folders — **7 Themes, 10 Widgets, 1 Control**. The full enumeration (key, kind, version,
assembly, packed artifact, mount, composition edges) is L5 canon in
...
|---|---|---|
| Full solution compiles | ✅ | `dotnet build -c Release MindAttic.Ideas.Library.slnx` → **Build succeeded, 0 Warning(s), 0 Error(s)**; all 18 component DLLs + Abstractions emitted (net10.0, SDK 10.0.300). |
| Smallest widget builds standalone | ✅ | `dotnet build -c Release Widgets/HelloWorld` → succeeded, 0/0. |
| Packed artifacts present | ✅ | [`dist/`](../dist) holds 18 `*.idea` — one per catalogued component in [`components.json`](data/components.json). |
...
| `pack` round-trip | 🟡 | `dist/*.idea` exist and predate this audit; the pack command (README) was **not** re-run during this Codex pass. |
| Plugin→Widget rename | 🟡 | Source renamed `Plugins/ → Widgets/` and Plugin→Widget in namespaces/types (staged in the working tree); a few README/`Pages/_wip` mentions of "Plugin" remain as historical prose. |
...
- Terminology cleanup: retire remaining "Plugin" prose now that the type is "Widget".
